...
import 'package:booking_system_flutter/model/my_home_response.dart';
import 'package:booking_system_flutter/model/notification_model.dart';
import 'package:booking_system_flutter/model/plan_response.dart';
...
Future<ConfigurationResponse> getAppConfigurations(
  ...
  try {
    ...
    if (data.otherSettings != null) {
      ...
      otherSettingStore
          .setPostJobRequestEnable(otherSetting.postJobRequest.validate());
      ...
      compareValuesInSharedPreference(IS_ADVANCE_PAYMENT_ALLOWED,
          otherSetting.isAdvancedPaymentAllowed.validate());
    }
...
Future<ConfigurationResponse> getTax({titleCheck}) async {
  try {
    ConfigurationResponse? data = ConfigurationResponse.fromJson(
...
Future<SingelProductResponse> getProductDetails(
    {required int ProductId, int? customerId, bool fromBooking = false}) async {
  if (fromBooking) {
    toast(language.pleaseWait);
  }

  try {
    var res = SingelProductResponse.fromJson(await handleResponse(
        await buildHttpResponse('store-item/$ProductId',
            method: HttpMethodType.GET)));
...
List<ProductModel> filterProducts({
  ...
  return products.where((product) {
    ...
    bool matchesProvider =
        providerId.isEmpty || product.providerId == providerId;
    bool matchesPriceMin =
        isPriceMin.isEmpty || product.price >= num.parse(isPriceMin);
    bool matchesPriceMax =
        isPriceMax.isEmpty || product.price <= num.parse(isPriceMax);
    bool matchesRating = ratingId.isEmpty ||
        product.status == ratingId; // Assuming status represents rating
    bool matchesSearch = search.isEmpty ||
        product.name.toLowerCase().contains(search.toLowerCase());
    bool matchesFeatured =
        isFeatured.isEmpty || product.isFeatured == isFeatured;
    bool matchesSubCategory =
        subCategory.isEmpty || product.subcategoryId == subCategory;
...
Future<List<ProductModel>> searchProductsAPI({
  ...
  try {
    var res = ProductResponse.fromJson(await handleResponse(
      await buildHttpResponse('store-item-list?$pages$perPages'),
    ));

    if (page == 1) list.clear();

    list.addAll(filterProducts(
        products: res.ProductList.validate(),
    ...
    print(list.length.toString() + ' length of products');

    lastPageCallBack?.call(list.validate().length != PER_PAGE_ITEM);
    appStore.setLoading(false);
  } catch (e, stackTrace) {
    appStore.setLoading(false);
...
Future<List<CategoryData>> getStoreCategoryListWithPagination(int page,
    {var perPage = PER_PAGE_CATEGORY_ITEM,
    required List<CategoryData> categoryList,
    Function(bool)? lastPageCallBack}) async {
  try {
    CategoryResponse res = CategoryResponse.fromJson(await handleResponse(
        await buildHttpResponse(
            'store-category-list?per_page=$perPage&page=$page',
            method: HttpMethodType.GET)));
...
Future<List<CategoryData>> getStoreSubCategoryListAPI(
    {required int catId}) async {
  try {
    CategoryResponse res = CategoryResponse.fromJson(await handleResponse(
        await buildHttpResponse(
            'store-subcategory-list?category_id=$catId&per_page=all',
            method: HttpMethodType.GET)));
...
Future<List<OrderData>> getOrderList(int page,
    {var perPage = PER_PAGE_ITEM,
    String status = '',
  ...
  try {
    ...
    if (status == BOOKING_TYPE_ALL) {
      res = OrderListResponse.fromJson(await handleResponse(
          await buildHttpResponse(
              'store-order-list?&per_page=$perPage&page=$page',
              method: HttpMethodType.GET)));
    } else {
      res = OrderListResponse.fromJson(await handleResponse(
          await buildHttpResponse(
              'store-order-list?status=$status&per_page=$perPage&page=$page',
              method: HttpMethodType.GET)));
    }
...
Future<BaseResponseModel> saveOrderPayment(Map request) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('store-order-save-payment',
          request: request, method: HttpMethodType.POST)));
}

Future<BaseResponseModel> saveSubscribtionOrderPayment(Map request) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('user-subscription-order-save-payment',
          request: request, method: HttpMethodType.POST)));
}
...
Future<MyHomeData> getMyhome(int page,
    {var perPage = PER_PAGE_ITEM,
    required MyHomeData? myHome,
    Function(bool)? lastPageCallBack}) async {
  try {
    MyHomeResponse myHomeResponse =
        MyHomeResponse.fromJson(await (handleResponse(await buildHttpResponse(
      'user-myhome',
      method: HttpMethodType.GET,
    ))));

    myHome = myHomeResponse.data!;
...
Future<List<PlansData>> getPlanlist(int page,
    {var perPage = PER_PAGE_ITEM,
    required List<PlansData> plans,
    Function(bool)? lastPageCallBack}) async {
  try {
    PlansResponse plansResponse = PlansResponse.fromJson(await (handleResponse(
        await buildHttpResponse(
            'user-subscription-plan-list?per_page=$perPage&page=$page',
            method: HttpMethodType.GET))));

    if (page == 1) plans.clear();
    plans.addAll(plansResponse.plansList.validate());

    lastPageCallBack
        ?.call(plansResponse.plansList.validate().length != PER_PAGE_ITEM);

    cachedPlanList = plans;
    appStore.setLoading(false);
  } catch (e, stackTrace) {
    appStore.setLoading(false);
...
Future<PlansData> getMyPlan(int page,
    {var perPage = PER_PAGE_ITEM,
    required PlansData? plan,
    required int plan_id,
    Function(bool)? lastPageCallBack}) async {
  try {
    MyPlanResponse plansResponse = MyPlanResponse.fromJson(
        await (handleResponse(await buildHttpResponse(
            'user-subscription-plan/$plan_id',
            method: HttpMethodType.GET))));

    plan = plansResponse.plan ?? null;
    cachedPlanList?.clear();
    cachedMyPlan = plan;
    appStore.setLoading(false);
  } catch (e, stackTrace) {
    appStore.setLoading(false);
...
Future<List<HistoryPlanData>?> getHistoryPlan(int page,
    {var perPage = PER_PAGE_ITEM,
    required List<HistoryPlanData>? plan,
    Function(bool)? lastPageCallBack}) async {
  try {
    HistoryPlanResponse plansResponse = HistoryPlanResponse.fromJson(
        await (handleResponse(await buildHttpResponse(
            'user-subscription-order?per_page=$perPage&page=$page',
            method: HttpMethodType.GET))));

    if (page == 1) plan == null;
    plan = plansResponse.plan ?? null;

    lastPageCallBack?.call(1 != PER_PAGE_ITEM);
...
Future<BaseResponseModel> createSubscribtionPlan(request) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('user-subscription-order',
          method: HttpMethodType.POST, request: request)));
}
