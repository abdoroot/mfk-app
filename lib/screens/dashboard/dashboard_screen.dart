...
import 'package:booking_system_flutter/screens/store/category_store_screen.dart';
    ...
    if (isMobile && appStore.isLoggedIn) {
      ...
      OneSignal.Notifications.addClickListener((notification) async {
        ...
        if (notification.notification.additionalData!.containsKey('id')) {
          ...
          print((notification.notification.additionalData!["type"] ==
                          'update_user_subscription_order_status' &&
                      notId.isNotEmpty)
                  .toString() +
              ' test validation notification');
          if (notification.notification.additionalData!["type"] ==
                  'update_user_subscription_order_status' &&
              notId.isNotEmpty) {
            PlansScreen(
              allPlans: true,
              singelPlanId: notId.toString().toInt(),
            ).launch(context);
          } else if (notId.validate().isNotEmpty) {
